...
class customfields extends db {
	...
	function get_next_category_sort_index($type) {
		$index = 1;
		$sql = "SELECT max(sort_index) AS sort_index FROM cf_categories WHERE extends_model='".$this->escape($this->CF_MODEL_TYPES[$type])."'";
		$this->query($sql);
		if($this->next_record()) {
	...
	function get_categories($type) {
		$sql = "SELECT * FROM cf_categories WHERE extends_model='".$this->CF_MODEL_TYPES[$type]."' ORDER BY sort_index ASC";
		$this->query($sql);
	...
	function get_authorized_categories($type, $user_id) {
		...
		$sql ="SELECT DISTINCT c.* ".
				...
				"WHERE extends_model='".$this->escape($model->tableName())."' AND (a.user_id=".intval($user_id)." ".
	...
	*/
	function get_authorized_fields($user_id, $link_type) {
		...
		$sql ="SELECT DISTINCT f.*, c.name AS category_name ".
				...
				"INNER JOIN go_acl a ON c.acl_id=a.acl_id ".
				"LEFT JOIN go_users_groups ug ON a.group_id=ug.group_id ".
				"WHERE c.extends_model='$link_type' AND (a.user_id=".intval($user_id)." OR ug.user_id=".intval($user_id).") ".
				" ORDER BY c.name ASC";
		$this->query($sql);
